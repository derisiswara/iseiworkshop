...
wvs_data <- read_excel("data/regresi/wvs1.xlsx")  
head(wvs_data)
```

Pastikan file xlsx tersimpan di folder data Anda! (Tidak ada auto save)
...
# Baca file WVS (sudah diterjemahkan ke Bahasa Indonesia)
library(readxl)
wvs_data <- read_excel("data/regresi/wvs1.xlsx")
glimpse(wvs_data)
```
...
> Muat file `data/regresi/wvs2.xlsx`, gabungkan dengan `wvs_data` menggunakan `bind_rows()`. Saring hanya responden dari Indonesia, hitung rata-rata `kepuasan_hidup` per `jenis_kelamin`.
